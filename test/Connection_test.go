...
func makeRepeatedBytes(n int) []byte {
	rand.Seed(time.Now().UTC().UnixNano())
	b := make([]byte, n)
	// A rand.Int63() generates 63 random bits, enough for letterIdxMax letters!
	letter := rand.Int63()
	for i, cache, remain := n-1, letter, letterIdxMax; i >= 0; {
		if remain == 0 {
			cache, remain = letter, letterIdxMax
		}
		if idx := int(cache & letterIdxMask); idx < len(letterBytes) {
			b[i] = letterBytes[idx]
			i--
...
func TestMaxAge(t *testing.T) {
	...
	_, err = connection.Download(ctx, _nfn, "/tmp/"+nfn, 1024)
	defer os.Remove("/tmp/" + nfn)
	assert.Nil(t, err)
	_, err = connection.Upload(ctx, "/tmp/"+nfn, nfn, 1024)
	assert.Nil(t, err)
	info, err = connection.DSEInfo(ctx)
	assert.Nil(t, err)
	...
	assert.Nil(t, err)
	for i := 0; i < 10; i++ {
		_, err = connection.Upload(ctx, "/tmp/"+nfn, fmt.Sprintf("file%d", i), 1024)
		if err != nil {
